  ...
  return (
    <>
      <div className='sideBar d-flex flex-column justify-content-start align-items-center border-end border-1 border-dark pt-3 fixed-top' style={{ width: sideWidth }}>
        ...
        <Link to="/my-projects" className='SidebarLinks p-3 d-md-flex align-items-center' title='My Projects'><FontAwesomeIcon icon={faFolderOpen} /> <span className={sideWidth === "10%" ? 'd-none d-md-block mx-1' : 'd-none mx-1'}>My Projects</span></Link>
        <Link to="/account-details" className='SidebarLinks p-3 d-md-flex align-items-center' title='My Account'><FontAwesomeIcon icon={faUserGear} /><span className={sideWidth === "10%" ? 'd-none d-md-block mx-1' : 'd-none mx-1'}>My Account</span></Link>
      </div>
    </>
  )
}
